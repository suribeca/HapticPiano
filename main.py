...
sensor_map = {
    "LED1": lambda: FSR_PINS["LED1"].read_u16() >= 18000, #Antes 52428
...
def get_estado_presionado():
    ...
    for led_name in leds:
        estado[get_dedo(led_name)] = sensor_map[led_name]()
    return estado
...
def on_mqtt_message(topic, msg):
    try:
        ...
        for finger, info in data.items():
            pressed = info.get("pressed")
            color = info.get("color")
            freq = info.get("freq", 0)  # default a 0 si no viene
            if not isinstance(freq, int):  # previene errores por tipos incorrectos
                freq = 500
            led_name = dedo_a_led(finger)
            if led_name and led_name in leds:
                color_rgb = hex_to_rgb565(color)
                if pressed:
                    set_led_color(leds[led_name], color_rgb)
